...
def plot_prop_for_col(
    ...
    prop_table = (
        plot_df.groupby(["dataset_group", "best_strategy"]).size().unstack(fill_value=0)
    )
    prop_table = prop_table.div(prop_table.sum(axis=1), axis=0).fillna(0)
    ...
    if prop_table.empty:
        ...
        if scale_to_percent:
            ax.set_ylabel("% of design spaces", fontsize=11)
            ax.set_ylim(0, 100)
        else:
            ax.set_ylabel("Proportion of \n design spaces", fontsize=11)
        if title is not None:
            ax.set_title(title, fontsize=11, pad=18 if subtitle else 8)
